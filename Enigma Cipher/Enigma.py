...
#
# This file is byte-for-byte identical to the top-level Enigma.py one folder
# up; the two are duplicate copies of the same script, not divergent
# versions. index.html (also one folder up) is a separate, self-contained
# browser front end: its embedded JavaScript reimplements this same cipher
# logic (rotor wiring, notch stepping, reflector, plugboard) so the machine
# can run in a page, with no runtime dependency between it and this script.
